...
//     offset_update_time);
static void
attack_spy_master(Matrix* mtr, size_t offset, useconds_t offset_update_time);
static void attack_slave(libflush_session_t* libflush_session, uint8_t* m,
    ...
    FILE* logfile);
static void 
printResult(Matrix* mtr,uint64_t* shared_addresses_hits);
...
int main(int argc, char* argv[])
{
  ...
  shared_addresses_hits = mmap(NULL, sizeof(shared_data), PROT_READ | PROT_WRITE,
      MAP_SHARED, shared_addresses_hits_shm_fd, 0);
...
attack_spy_master(Matrix* mtr, size_t offset, useconds_t offset_update_time)
{
  do {
    ...
    // 分析数据得到结果
    printResult(mtr,shared_addresses_hits);
  } while (true);
}
  ...
  do {
    while (current_offset == shared_data->current_offset) {
      ...
      for (unsigned int i = 0; i < number_of_tests; i++) {
        ...
        if (hit_counter > shared_addresses_hits[shared_addresses_hits[0]]){
          shared_addresses_hits[shared_addresses_hits[0]] = hit_counter;
        }
...
{
  int* chars = NULL;
  int cc = getCharacters(mtr,shared_addresses_hits+1,chars);
  for (int i = 0; i < cc; i++)
  {
