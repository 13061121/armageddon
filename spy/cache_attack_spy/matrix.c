...
size_t
getValue(uint64_t num, uint64_t thres, uint64_t threshold){
    if (num<=thres)
        return 0;
...
bool
isCharacter(size_t* values, int vc, size_t* valueline){
    int i;
    size_t t;
    for(i=0;i<vc;++i){
        t = values[i];
        if ((t == 0 && valueline[i]!=0) || ((t&valueline[i]) == 0 ))
            return false;
    }
    return true;
}
...
int
getCharacters(Matrix* mtr, uint64_t* values,int* chars){
    unsigned int i, j;
    int charcount;
    bool* characters;
...
void
printMatrix(FILE * const _Stream, Matrix *mtr){
    if(mtr->ac !=0)
    ...
    if(mtr->thresholds != NULL){
        ...
        for(unsigned int i=0;i<mtr->ac;++i){
            fprintf(_Stream, "%d\t",mtr->thresholds[i]);
        }
        fprintf(_Stream, "\n");
    }
